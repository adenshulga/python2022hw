...
class List:
    ...
    def __getitem__(self, i):
        if i < 0 or i >= self._length:
            return False
        if i <= len(self)/2: # начинаем идти до элемента сначала
            curr_pointer = self._start_pointer
            for j in range(i):
                curr_pointer = curr_pointer.get_next()
        else: # идем к элементу с конца, надо заменить лишь указатель на начало указателем на конец, и идти назад а не вперед
